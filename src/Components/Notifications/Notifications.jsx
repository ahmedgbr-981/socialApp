...
import markAllasRead from "../../api/markAllasRead.api";
import markNotificationsRead from "../../api/markAsRead.api";
...
export default function Notifications() {
  ...
  const {
    data: markReadData,
    isPending: markReadIspen,
    mutate,
  } = useMutation({
    mutationFn: (nId) => markNotificationsRead(nId),
    onSuccess: (markReadData) => {
      console.log("patched notis", markReadData);
      q.invalidateQueries({
        queryKey: ["notifications"],
      });
      q.invalidateQueries({
  ...
  const {
    ...
    isPending: markAllReadIspen,
    mutate: markAllMutate,
  } = useMutation({
    mutationFn: () => markAllasRead(),
    onSuccess: (markAllReadData) => {
      console.log("mark all notis", markAllReadData);
      q.invalidateQueries({
        queryKey: ["notifications"],
      });
    },
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <section className="overflow-hidden rounded-[28px] border border-slate-800 bg-slate-900/80 shadow-2xl shadow-slate-950/60 backdrop-blur-xl">
          <header className="flex items-center justify-between border-b border-slate-800 bg-slate-900/70 px-5 py-4 sm:px-6">
            ...
            <button
              type="button"
              onClick={() => markAllMutate()}
              className="hidden cursor-pointer rounded-full border border-slate-700 bg-slate-800 px-3 py-1.5 text-sm font-medium text-slate-200 transition hover:border-sky-500/60 hover:text-white sm:inline-flex"
            >
              Mark all readll
            </button>
          </header>
          ...
          {!isLoading && !isError && data.length === 0 && (
            <div className="flex min-h-60 flex-col items-center justify-center px-6 py-10 text-center">
              ...
              <p className="mt-2 max-w-md text-sm text-slate-400">
                When someone likes, follows, or comments on your posts, they
                will appear here.
              </p>
            </div>
          )}
            ...
            <div className="divide-y divide-slate-800">
              {data.map((notification, index) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`mt-0.5 flex h-12 w-12 items-center justify-center rounded-2xl border ${meta.color}`}
                    >
